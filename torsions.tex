\section{Кручения в некоторых тензорных произведениях модулей}
    ...
    \begin{Def}
        Пусть $M$ -- произвольный $A$-модуль. \textit{Подмодулем кручения} $\tors{M}$ называется множество
        \begin{equation*}
    ...
    Заметим, что сумма \eqref{sl} является прямой, так как $I^s_t$ -- подмодуль $A$-модуля $A[t]$.
    ...
    \begin{Proof}
        ...
        коэффициентами из $A$. Значит он не даст вклада в кручения и его можно вынести за знак $\tors{\cdot}$.
        Таким образом имеем
        \begin{multline*}
        ...
        \end{equation*}
    \end{Proof}
    Задача свелась к вычислению $\tors{I^s \ox J}$. Пусть $J = I$, тогда справделива следующая 
