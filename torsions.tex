\section{Кручения в некоторых тензорных произведениях модулей}
    ...
    Рассмотрим следующую задачу. Описать кручения $\hat A$-модуля $M \ox_A \widehat{A}$, если 
    ...
    \end{equation*}
    где $I_1, I_2 \subset A$ --- идеалы в кольце $A$, $A$ --- целостное, нетерово. 

    Обозначим $\widehat{M} := M \ox_A \widehat{A}$. Так как тензорное умножение не точно слева, то
    ...
    $$   
        \xymatrix{
            ...
            & \Tors{\hat A}{I_2} \ar[r] \ar@{^{(}->}[u]
            &0
        }
    $$
    где гомоморфизмы в нижней строке получены путем ограничения гомоморфизмов верхней строки на соответствующие множества. 
    Разложим гомоморфизм $\hat i'$ в композицию сюръективного и инъективного морфизмов и рассмотрим нижнюю строку
    $$   
        \xymatrix{
            0 \ar[r] 
            & \Tors{\hat A}{\tau} \ar[r] 
    ...
    $$
    Отметим, что $\hat I_1, \hat I_2$ конечно порождены, как $\hat A$-модули, поэтому $\Tors{\hat A}{\hat I_2}$
    конечно порожден как подмодуль нетерового модуля, $\frac{\Tors{\hat A}{\hat I_1}}{\Tors{\hat A}{\tau}}$
    конечно порожден как образ конечно порожденного модуля.
    Поэтому мы можем воспользоваться предложением 4 $\S 4$ гл. 1 книги 
    \textbf{[источник]}, которое утверждает, что расширение последовательности 
    \begin{equation*}
    ...
    --- произвольно выбранный проообраз $\bar{z_i}$ $(i =\bar{1, m})$ и 
    ...
    Теперь выясним как охаракетризовать кручения произвольного $\hat A$-модуля \\$M \ox_A \hat A$, при 
    условии что $M$ --- нетеров $A$-модуль. Для этого нам потребуются известные утверждения:
    \begin{Theorem} \label{Mvee1}
        $M^{\vee} := \Hom_A(M, A)$ --- модуль без кручения.
    \end{Theorem}
    \begin{Theorem} \label{Mveevee}
        Если $M$ --- модуль без кручения, то гомоморфизм $M \rightarrow M^{\vee \vee}$ инъективен.
    \end{Theorem}
    ...
    \begin{equation*}
        0 \rightarrow N^\vee \rightarrow M^{\vee \vee} \rightarrow A.
    \end{equation*}
    ...
    \end{equation*}
    Воспользовавшись теоремой \ref{Mveevee} имеем вложение
    \begin{equation*}
        \xymatrix{
            0 \ar[r]
            &N^\vee \ar[r] 
    ...
    \end{equation*}
    Таким образом, $M_1 \subset M$, $M$ --- нетеров, следовательно, $M_1$ тоже нетеров. Повторим эти же
    ...
    что это не так и цепочка будет обрываться. Перейдем к локализации в нулевом идеале кольца $A$. 
    ...
    \end{equation*}
    Так как $(J_{i + 1})_0 \simeq A_0$, то из свойсва аддитивности $A_0$-размерности имеют место равенства
    \begin{equation*}
        \dim_{A_0} (M_i)_0 - \dim_{A_0}A_0 = \dim_{A_0} (M_i)_0 - 1 = \dim_{A_0}(M_{i+1})_0.
    \end{equation*}
    Таким образом, последовательность \eqref{subsets} действительно обрывается и базой будет являться 
    случай, в котором имеем точную тройку вида
    \begin{equation*}
