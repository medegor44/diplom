\section{Кручения в некоторых тензорных произведениях модулей}
    ...
    \begin{Def}
        Пусть $I \subset A$ --- идеал. \textit{Алгеброй раздутия} идеала $I$ назовем выражение 
    ...
    \end{Def}
    \begin{Def}
        Будем говорить, что $A$-модуль $M$ является \textit{модулем без кручения}, если $\tors{M} = 0$.
    \end{Def}

    Заметим, если $A$ не является целостным кольцом, то $\tors{M}$ не обязательно является подмодулем
    в $M$.
    ...
    \begin{Proof}
        ...
        \begin{equation*}
            ...
            \Bigg| a_n, b_m \in A, n = \bar{0, r}, m = \bar{0, s}\right\}.
        \end{equation*}
        Теперь, раскрывая скобки, получим
        \begin{multline*}
            I^{r + 1} = \{ b_0a_0y^{r + 1} + (b_1a_0 + b_0a_1)xy^{r} + \dots \\+ 
                (b_1a_{r - 1} + b_0a_{r})x^ry + b_1a_rx^{r + 1}  | a_n, b_m \in A, n = \bar{0, r}, m = \bar{0, s} \}
        \end{multline*}
    ...
    \begin{Proof}
        ...
        только конечное число колпонент $t_j$ отлично от нуля. Так как умножение на элементы прямой суммы
        производится покомпонентно, то 
        \begin{equation*}
    ...
    \begin{Proof}
        ...
        \begin{eqnarray*}
            z = a_0(x^s \ox x) &+& a_1(x^{s - 1}y \ox x) + \dots + a_s(y^s \ox x) + \\
                               ...
                               b_{s + 1}(y^s \ox y),
        \end{eqnarray*}
        где $a_i, b_i \in A$. $\mu(z)$ будет иметь следующий вид:
        \begin{equation*}
    ...
    \begin{Proof}
        ...
        Рассмотрим образующую $x^ny^{s - n} \ox x^{k - n}y^{r - k + n}$ и умножим на 
        $x^ry^r$. Имеем
        \begin{multline*}
        ...
        \begin{equation*}
            x^ry^r\sum_{k=0}^{s + r}\sum_{n = 0}^{\min(s, k)} a_{nk-n}x^ny^{s - n} \ox x^{k - n}y^{r - k + n} =
            \sum_{k=0}^{s + r} \left[ \left(\sum_{n = 0}^{\min(s, k)}a_{nk-n}\right)x^ky^{r + s - k} \ox x^ry^r \right] = 0,
        \end{equation*}
    ...
    \end{Theorem}
    \begin{Proof}
        Действительно, рассмотрим элемент $I^s_t$ следующего вида 
        ...
        \end{equation*}
        то есть, мы показали, что элементы вида \eqref{rightel} действительно дают вклад в кручение.
